...
Our Github page: https://github.com/TomMakesThings/Genomics-II-Group
...
Tumor mutational burden (TMB) was calculated for each sample as the total number of nonsynonymous mutations, divided by the number of bases sequenced. 
...
## set the variant_classification that count as "nonsynonymous mutations", in total 9 types

variant_classification <- unique(mut$variant_classification)

no_variant_class <- c("5'Flank", "Translation_Start_Site", "3'Flank",
                      "Silent", "5'UTR", "IGR","Intron") #does not count as "nonsynonymous mutations", 7 types
yes_variant_class <- variant_classification[!variant_classification %in% no_variant_class] # count as "nonsynonymous mutations", 9 types
...
Spearman's correlation was performed on oncogenic alteration data (amplification, deletion, mutation) and MB. The significant results were further filtered with multivariable linear modeling to exclude the effect from sample subtype (primary/metastatic).

<<echo = T, eval = F>>=
#############Run spearman on oncogenic alterations (amp, del, mut)###############
#############then multi-variablelinear model#######
...
run_lm <- function(df_spearman, cna_df){ 
...
ggsave("plot/fig3c.png", plot = final, width = 6, height = 15)
ggsave("plot/fig3c.pdf", plot = final, width = 6, height = 15)
