...
amp_spearman_sig <- subset(amp_spearman_sig,!amp_spearman_sig$genes %in% c("CCND1","FGF19"))
del_spearman_sig <- subset(del_spearman_sig,!del_spearman_sig$genes %in% c("CCND1","FGF19"))


#remove FOXA1  Lung Adenocarcinoma; CCND1_FGF19 Ovarian High-Grade Serous;
out_amp <- amp_spearman_sig[ -c(2,4),]

#add CDKN2A Bladder Urothelial into del, 
out_del <- rbind.data.frame(del_spearman_sig,
                            del_lm[which(del_lm$tumor_type=="Bladder Urothelial" & del_lm$genes =="CDKN2A"),]
                            )
