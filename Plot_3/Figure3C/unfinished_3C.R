...
onco_alterations <- subset(table_s3b, table_s3b$alteration_type=="oncogenic alteration")


clinical_sample <- read.table("data/data_clinical_sample.txt", sep = "\t",header = TRUE)
cna <- read.table("data/data_cna.txt", sep = "\t",header = TRUE)

myc <- read.table("cBioPortal/myc.txt", sep = "\t", header = TRUE)
mut <- read.table("cBioPortal/mut.txt", sep = "\t", header = TRUE)

##get useful column: to map sampleID and cancer subtype
sampleID_subtype <- clinical_sample[c("SAMPLE_ID", "PATIENT_ID","SUBTYPE", "SUBTYPE_ABBREVIATION",
                                      "MET_SITE_COUNT")]

##add cancer subtype, MB into cna table
##all((myc$SAMPLE_ID)==sampleID_subtype$SAMPLE_ID) same order of sampleID confirmed
myc <- cbind.data.frame(myc, SUBTYPE_ABBREVIATION = sampleID_subtype$SUBTYPE_ABBREVIATION,
...
##test calculate myc-amplification and mb in Prostate Adenocarcinoma

myc <- subset(myc, myc$MET_SITE_COUNT > 0)
Pro_Ade <- subset(myc, myc$SUBTYPE == "Prostate Adenocarcinoma") #nrow=1762!!, =n_pts in sup table

myc_amp <- Pro_Ade$MYC # either 0 or 2

SpearmanRho(x=myc_amp, y=Pro_Ade$MET_SITE_COUNT, conf.level = 0.95) #get coef=0.1168 ~0.12

pten_del <- Pro_Ade$PTEN # either -2, 0, 2
pten_del[which(pten_del == 2)] <- 0 #all 2 to 0, either -2, 0
pten_del[which(pten_del == -2)] <- 2

SpearmanRho(x=pten_del, y=Pro_Ade$MET_SITE_COUNT, conf.level = 0.95) #get coef=0.16126

#pten_del
#-2    0    2 
#267 1494    1 

#我们：PTEN mutation1787 rows, mutations table中，有2339 rows. 因此，要计算个数
## 如果是“Q219*(Driver), P103Tfs*4(Driver)， 就算作2， 在mutations table中占据两列

#length(unique(mutations$Hugo_Symbol)) = 492 small, 在mutations表中有492个基因. 里面有一些hugo symbol好像不是gene 
#nrow(cna) = 541 在cna大表格中有541个基因. big, 应该用这个，更接近excel S3里面的名字，而且可以直接搜索。但是都是oncogene吗？
#从excel里面直接extract出来的,注意HLA-A 这种他换成了., underscore之类,共178个基因，应该这些都是oncogene


##get download list of genes (oncogenes)
gene.list <- strsplit(onco_alterations$alteration, "_(?!.*_)", perl = TRUE)
gene.list <- sapply(gene.list, function(x){
    return(x[1])
})##NKX2_1_FOXA1 实际上是 NKX2-1, FOXA1两个基因
##

##gene from cBioPortal --CNA genes
CNAgenes <- read.table("cBioPortal/CNA_Genes_all.txt", sep = "\t", header = FALSE, skip = 1)
colnames(CNAgenes) <- c("Gene", "Gistic(Q-value)", "Cytoband", "CNA",
                        "Profiled_Samples", "Num","Freq", "Is_Cancer_Gene(source_OncoKB)") #928 total rows, 468 unique genes
#因为有些基因发生了多种alteration, 包括amp, del

##gene from cBioPortal --mutated genes
mutgenes <- read.table("cBioPortal/Mutated_Genes_all.txt", sep = "\t", header = FALSE, skip = 1)
colnames(mutgenes) <- c("Gene", "Mutsig(Q-value)", "Mut","Num",
                        "Profiled_Samples", "Freq", "Is_Cancer_Gene(source_OncoKB)") #477 total rows, 477 unique genes


PTEN_mut <- mut[which(mut$PTEN!="WT"),]
